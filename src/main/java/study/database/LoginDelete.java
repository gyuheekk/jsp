...
public class LoginDelete extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 현재 로그인한 정보와 삭제하려는 mid가 같은지를 비교해서 같으면 자신의 정보를 삭제 후 로그아웃처리, 다르면 다시 회원리스트 창으로 보낸다.
		...
		if(!mid.equals(sMid)) {
			...
			out.print("location.href='"+request.getContextPath()+"/study/database/Logout';");
			out.print("</script>");
		}
	}
}
