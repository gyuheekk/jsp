...
//@WebFilter("/*")
public class FiltertCertification implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		chain.doFilter(request, response); //관문통과
		
		if(admin.equals("admin") && aCertification.equals("")) {
			out.println("<script>");
			out.println("alert('관리자님 인증코드를 발급해주세요.');");
			// out.println("location.href='t2_Certification.jsp?admin=admin'"); 필터안에서는 location.href 사용 불가
			out.println("</script>");
		}
