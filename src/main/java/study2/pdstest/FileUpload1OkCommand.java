...
/*
자바 파일업로드시...(컨트롤러에 아해 어노테이션에 환경설정처리한다.)
@MultipartConfig(
		location = "/images/pdstest",
		maxFileSize = 1024 * 1024 * 10,l
...
public class FileUpload1OkCommand implements StudyInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse reponse) throws ServletException, IOException {
		...
		/*
		  ...
		  사용법 : MultipartRequest(저장소명(request),'서버에 저장될 파일의 경로','서버에 저장될 파일의 최대용량','코드변환방식','기타옵션(파일명 중복방지 처리)) 앞 4개는 필수
		...
		*/
		...
		int maxSize = 1024 * 1024 * 10; // 서버에 저장시킬 파일의 최대용량 : 10MByte로 제한(1회 저장 용량)
